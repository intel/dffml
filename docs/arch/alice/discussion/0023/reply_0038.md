...
subgraph shared_key_between_did_and_ssh_key[Shared key between DID and ssh key]
  ...
  file -->|Alice creates DID doc for file| alice_root_for_everything --> http_server
  ...
  verify_message_bob_root_for_everything --> |Message data contains github id| github
  github --> public_key[Public key stored in GitHub] --> bob_root_for_everything
  bob_root_for_everything --> http_server
end
...
subgraph ephemeral_ssh_key_for_did_signing_as_credential[Ephemeral ssh key for DID signing as credential]
  ...
  verify_message_bob_github_proof --> |Message data contains github id| github
  github --> public_key[Public key stored in GitHub] --> bob_distinct_root
  bob_distinct_root --> http_server
end

subgraph bob_pool[Bob shared pool]
