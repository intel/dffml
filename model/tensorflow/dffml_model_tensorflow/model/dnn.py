...
class DNNContext(ModelContext):
    ...
    def mkclassifications(self, classifications):
        classifications = {
            value: key for key, value in self.mkcids(classifications).items()
        }
        LOGGER.debug(
    ...
    async def train(
        ...
        (await self.model(features, classifications)).train(
    ...
    async def predict(
        ...
        input_fn, predict = await self.predict_input_fn(
            repos, features, classifications
        )
        # Makes predictions on classifications
        predictions = (await self.model(features, classifications)).predict(
            input_fn=input_fn
        )
