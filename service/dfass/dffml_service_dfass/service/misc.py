...
'''
Docstring for this miscellaneous service.
'''
from typing import Type

from dffml.service import Feature

from .log import LOGGER

class MiscFeature(Feature):
    '''
    Explination of this service for users of the CLI
    '''
    ...
    async def applicable(self, data) -> bool:
        '''
        Check if it makes sense to run the service on this key
        '''
    ...
    async def fetch(self, data):
        ...
        '''
        await data.data.set('dfass_misc',
                            '%s + dfass_misc' % (data.src_url,))
    ...
    async def calc(self, data):
        '''
        Calculates the score for this service based on data found by parse().
        '''
        return await data.data.get('dfass_misc')
