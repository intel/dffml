...
class TestOperations(AsyncTestCase):
    async def setUp(self):
        ...
        self.dataflow.seed.append(
            Input(
                value=[restart_running_containers.op.outputs["containers"].name],
    ...
    async def test_0_start_container(self):
        with mock.patch.object(clone_git_repo.imp,'CONTEXT',new = FakeCloneRepoImp):
            tag = f"{USER}/{REPO}"
            before = await check_output("docker","ps","--filter",f"ancestor={tag}", "--format","{{.ID}} {{.RunningFor}}" )
            async with MemoryOrchestrator.withconfig({}) as orchestrator:
                async with orchestrator(self.dataflow) as octx:
                    async for ctx, results in octx.run(self.test_inputs):
                        after = await check_output("docker","ps","--filter",f"ancestor={tag}", "--format","{{.ID}} {{.RunningFor}}" )
                        self.assertNotEqual(before,after)
                        self.assertIn('docker_restarted_containers',results)
                        self.containers_to_remove = results['docker_restarted_containers']
