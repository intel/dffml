...
class XLSXSource(FileSource, MemorySource):
    ...
    async def load_fd(self, fd):
        ...
        max_columns = excel_sheet.max_column # Total Columns in the excel sheet
        for row in range(2, max_rows+1): # Iterate over each record rows excluding the features
            temp_dict = {}
            for col in range(1, max_columns+1):
                feature = excel_sheet.cell(row = 1, column = col) # Features data
                cell_element = excel_sheet.cell(row = row, column = col) # Cell content corresponding to the feature data
                temp_dict[str(feature.value)] = cell_element.value
            self.mem[str(row-1)] = Record(str(row-1), data={"features": temp_dict})
    ...
    async def dump_fd(self, fd):
        ...
        for section, records in self.mem.items(): # Iterating over each row
            ...
            if not features: # If the fetures list is empty add the features, and then append it to the excel_sheet 
