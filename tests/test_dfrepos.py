...
from dffml.operation.dfrepos import (
    run_dataflow_on_repo,
    RunDataFlowOnRepoConfig,
)
...
class TestRunDataFlowOnRepo(IntegrationCLITestCase):
    async def test_run(self):
        ...
        with relative_chdir("..", "..", "examples", "shouldi"):
            ...
            shouldi_dataflow = DataFlow._fromdict(
                **json.loads(write.call_args[0][0])
            )
        ...
        test_dataflow = DataFlow(
            operations={
                "run_on_repos": run_dataflow_on_repo.op,
                "get_single": GetSingle.imp.op,
            },
            configs={
                "run_on_repos": RunDataFlowOnRepoConfig(shouldi_dataflow)
            },
            seed=[
                Input(
        ...
        test_ins = [
            {
                "insecure_package": [
                    {"value": "insecure-package", "definition": "package"}
                ]
            },
            {"dffml": [{"value": "dffml", "definition": "package"}]},
        ]
        test_outs = {
            "insecure_package": {
                ...
                async for _ctx, results in octx.run(
                    {
                        list(test_in.keys())[0]: [
                            Input(
                                value=test_in,
                                definition=run_dataflow_on_repo.op.inputs[
                                    "ins"
                                ],
                            )
                        ]
