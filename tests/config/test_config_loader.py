...
class TestConfigLoader(AsyncTestCase):
    ...
    def setUpClass(cls):
        ...
        pathlib.Path(cls.test_dir.name,cls.config_folder_name+".dirconf.json").write_text(
            json.dumps({"hello":"there"})
          )
        pathlib.Path(temp_path,'deadbeef.json').write_text(json.dumps({"massive":"hax"}))
        os.mkdir(os.path.join(temp_path,'feed'))
        pathlib.Path(temp_path, 'feed', 'face.json').write_text(json.dumps({"so":"secure"}))
